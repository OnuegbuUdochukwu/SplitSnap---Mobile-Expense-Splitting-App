...
export default function ScanScreen() {
  ...
  // Runtime enum values are numeric; provide runtime constants to avoid TS value/type confusion
  const FLASH_OFF = 0 as unknown as FlashMode;
  const FLASH_TORCH = 3 as unknown as FlashMode;
  const CAMERA_BACK = 1 as unknown as CameraType;
  const CAMERA_FRONT = 2 as unknown as CameraType;

  // Camera has problematic JSX typings in some environments; cast to any for rendering
  const CameraAny: any = Camera;

  const [flashMode, setFlashMode] = useState<FlashMode>(FLASH_OFF);
  const [cameraType, setCameraType] = useState<CameraType>(CAMERA_BACK);
  ...
  const toggleFlash = () => {
    setFlashMode((m: FlashMode) => (m === FLASH_OFF ? FLASH_TORCH : FLASH_OFF));
  };

  const flipCamera = () => {
    setCameraType((t: CameraType) =>
